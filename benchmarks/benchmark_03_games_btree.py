...
def run_benchmark():
    print("\n" + "="*80)
    print("Benchmark 03: Games - Índice B+Tree")
    print("="*80)
    ...
    for csv_path, size_label in datasets:
        ...
        tm = TableManager()
        table_name = f"games_btree_{size_label.lower()}"
        
        start = time.time()
        query = f"CREATE TABLE {table_name} FROM FILE '{csv_path}' USING PRIMARY INDEX BTREE(AppID), INDEX HASH(Name);"
