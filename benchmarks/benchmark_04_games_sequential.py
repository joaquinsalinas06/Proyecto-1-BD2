"""
Benchmark 04: Games Sequential File Index + Hash
...
def run_benchmark():
    """Execute complete benchmark for Games dataset with Sequential File + Hash"""

    print("\n" + "="*80)
    print("BENCHMARK 04: GAMES - SEQUENTIAL FILE INDEX + HASH")
    print("="*80)
    ...
    for csv_path, size_label in datasets:
        ...
        table_name = f"games_seq_{size_label.lower()}"
        ...
        try:
            ...
            if metadata_path.exists():
                try:
                    ...
                    tables_to_remove = [k for k in metadata.keys() if k.startswith('airbnb_seq_')]
            ...
            start = time.time()
            create_sql = f"CREATE TABLE {table_name} FROM FILE '{temp_bulk_csv}' USING PRIMARY INDEX SEQ(AppID), INDEX HASH(Name);"
            print(f"   📝 {create_sql}")
            tm.sql(create_sql)
    ...
    print("\n" + "="*140)
    print("RESUMEN DE RENDIMIENTO - SEQUENTIAL FILE + HASH INDEX")
    print("="*140)
    ...
    print("\n" + "="*140)
    print("RESUMEN DE ESTADÍSTICAS DE I/O - SEQUENTIAL FILE + HASH INDEX")
    print("="*140)
    ...
    print("   ✅ Sequential (AppID): Búsqueda exacta y rangos - INDEXADO")
